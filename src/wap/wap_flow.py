...
def run_quality_checks(
    bauplan_client: bauplan.Client, bauplan_ingestion_branch: str, table_name: str
):
    """
    ...
    wap_table = bauplan_client.scan(
        table=table_name, ref=bauplan_ingestion_branch, columns=[column_to_check]
    )
...
def wap_with_bauplan(
    ...
    with transaction():
        ...
        run_quality_checks(
            bauplan_client, bauplan_ingestion_branch, table_name=table_name
        )
        # THIS IS THE PUBLISH
...
if __name__ == "__main__":
    ...
    print(
        f"Starting the WAP flow with the following parameters: {table_name}, {branch_name}, {s3_path}"
    )
    # start the flow
    wap_with_bauplan(
